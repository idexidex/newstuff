...
// определяем номера столбцов по имени в выборке 
			...
			int nalichColIndex = c.getColumnIndex("nalich");
			int prihrashColIndex = c.getColumnIndex("prihrash");
			int commentColIndex = c.getColumnIndex("comment");
			int chIdColIndex = c.getColumnIndex("chId");
			// Леха :выше скопировал "nalich text,"+"prihrash text,"+"comment text,"+ "chId text"+");");
         
            do { 
                // получаем значения по номерам столбцов и пишем все в лог
 // Леха ниже nalich ,prihrash , comment, chId.
                Log.d(TAG, "ID = " + c.getInt(idColIndex) + " | " + c.getString(timeColIndex) + " | " + c.getString(dateColIndex)+" | " + c.getString(numericColIndex)+" | " + c.getString(teloColIndex)+" | " + c.getString(nalichColIndex) +" | " + c.getString(prihrashColIndex) +" | " + c.getString(commentColIndex)+" | " + c.getString(chIdColIndex));  
    ...
    public Cursor getCursor(String currentTable,Context context){
        dbHelper = new DBHelper(context);
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        Cursor c = db.query(currentTable, null, null, null, null, null, null, null); 
        //Леха выше добавил еще Один null, думаю это очистит поле comment при создании записи
        return c;
    }
